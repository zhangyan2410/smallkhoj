...
api key/token models, settings/admin UI, connect ticket/token APIs
...
API key spec: prefix-only list, create/rotate/revoke, one-time secret display, type/owner/audit safety.
...
```text
REAL_api_key_management_ui_<YYYYMMDDHHMMSS>
```
...
8. Write `evidence/REAL_api_key_management_ui_<timestamp>-notes.md` with commands run, pass/fail, and remaining gaps.
