...
void main(){
  ...
  class InkMaterialSurface {
    constructor(canvas, options = {}) {
      ...
      // seed initial marks so non-idle states show something immediately
      this.setState({ state: this.state, seed: options.seed, clear: false, seedMarks: true });
    ...
    stroke(points, opts = {}) {
      ...
      for (let i = 0; i < points.length; i++) {
        ...
        const spacing = Math.max(baseRadius * 0.6, 0.0008);
        const steps = Math.min(Math.max(Math.ceil(dist / spacing), 1), 80);
    ...
    _step(dt) {
      ...
      // of the last input), so the baked art stays put until deliberately washed.
      {
        ...
        const lift = (this.renderMode === "live" && recentBrush) ? 1.5 : preset.fixedLift;
