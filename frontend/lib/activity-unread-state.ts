 ...
 *
 * 语义：
 * - 由 SSE 事件驱动递增（见 activity-unread-tracker），同 scope 的 seq 高水位
 *   去重 —— 同一事件重放/重连不重复计数。
 ...
 * - message.created（channel/dm scope）：自己发的、当前正在查看的频道/DM 不计；
 *   其余递增聊天域键（键派生复用 chat-unread-state 的规则，由调用方传入）。
...
export function activityUnreadKeysForEvent(
  ...
  const { pathname, currentMemberNames = [], chatScopeKeys } = options
