...
daemon control APIs, workspace state, frontend computers/members controls
...
Runtime lifecycle spec: stop/restart/kill/reconcile controls, offline safety, workspace status reconciliation.
...
```text
REAL_runtime_lifecycle_controls_<YYYYMMDDHHMMSS>
```
...
8. Write `evidence/REAL_runtime_lifecycle_controls_<timestamp>-notes.md` with commands run, pass/fail, and remaining gaps.
