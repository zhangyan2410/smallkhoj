...
smallkhoj-trace, twd.py evidence, API/DB marker lookup, debug route/docs
...
Human debug workbench spec: marker-first guided flow linking browser, API, DB, trace, daemon/runtime, task evidence.
...
```text
REAL_human_debug_workbench_<YYYYMMDDHHMMSS>
```
...
8. Write `evidence/REAL_human_debug_workbench_<timestamp>-notes.md` with commands run, pass/fail, and remaining gaps.
