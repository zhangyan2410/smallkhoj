...
- Completion supplemental evidence now covers a persistent 390x844 mobile/
  coarse-pointer touch run, a real two-origin same-Name Human collision with
  qualified suggestions and compact runtime update, a real invitation return,
  a second-Channel provider baseline, and a deliberately failed Better Auth
  bridge followed by idempotent Name-only setup. See
