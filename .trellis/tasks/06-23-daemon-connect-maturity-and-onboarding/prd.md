...
- 旧 wrapper 清理日志现在包含 server URL、PID 和 lock 文件路径，便于判断 SIGTERM 是否来自同 server wrapper 替换。

## Verification 2026-06-23

- `bash -n smallkhoj-daemon` passed.
- `cd agent/daemon/aaa-daemon && rtk node --test test/smallkhoj-daemon-wrapper.test.mjs` passed: 3 tests.
- `cd agent/daemon/aaa-daemon && rtk npm run build` passed.
- `cd agent/daemon/aaa-daemon && rtk npm test` passed: 144 tests.
