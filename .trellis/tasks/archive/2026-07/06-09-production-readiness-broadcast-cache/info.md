...
backend event/control hub, EventRecord, daemon websocket/polling, deployment config
...
Production readiness spec: multi-instance gaps, broadcast/cache layer, rollout/rollback, local-dev simplicity.
...
```text
REAL_production_readiness_broadcast_cache_<YYYYMMDDHHMMSS>
```
...
8. Write `evidence/REAL_production_readiness_broadcast_cache_<timestamp>-notes.md` with commands run, pass/fail, and remaining gaps.
