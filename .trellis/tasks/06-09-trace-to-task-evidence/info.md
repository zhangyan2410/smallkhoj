...
smallkhoj-trace output, task evidence model/UI
...
Trace evidence spec: filtered marker trace, concise summary, raw path, task evidence linkage.
...
```text
REAL_trace_to_task_evidence_<YYYYMMDDHHMMSS>
```
...
8. Write `evidence/REAL_trace_to_task_evidence_<timestamp>-notes.md` with commands run, pass/fail, and remaining gaps.
