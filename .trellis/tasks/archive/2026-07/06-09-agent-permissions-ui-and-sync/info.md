...
member config/profile APIs, daemon/runtime config path
...
Permissions spec: editable permission groups, persisted config, runtime sync path, enforcement status honesty.
...
```text
REAL_agent_permissions_ui_and_sync_<YYYYMMDDHHMMSS>
```
...
8. Write `evidence/REAL_agent_permissions_ui_and_sync_<timestamp>-notes.md` with commands run, pass/fail, and remaining gaps.
