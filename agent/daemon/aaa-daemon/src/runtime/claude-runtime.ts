...
export function buildSlockSystemPrompt(options: Pick<ClaudeRuntimeOptions, 'credential' | 'workspacePath'> & Partial<Pick<ClaudeRuntimeOptions, 'wrapperDir'>>): string {
  ...
  return [
    ...
    '33. **`slock inbox check`** — Show pending inbox targets without draining or reading message content.',
    '34. **`slock channel mute` / `slock channel unmute`** — Mute or unmute ordinary Activity delivery for a regular channel.',
    '35. **`slock manual get` / `slock manual search`** — Fetch or search Raft operating manual topics and recipes.',
    '36. **`slock auth whoami`** — Print the local agent context resolved from env without exposing token values.',
    '',
