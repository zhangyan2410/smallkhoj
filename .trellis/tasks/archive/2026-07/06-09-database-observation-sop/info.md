...
docs, DBX/PostgreSQL, backend models/schema
...
DB observation SOP spec: safe read-only marker lookup across messages/tasks/events/activity with product-language explanations.
...
```text
REAL_database_observation_sop_<YYYYMMDDHHMMSS>
```
...
8. Write `evidence/REAL_database_observation_sop_<timestamp>-notes.md` with commands run, pass/fail, and remaining gaps.
