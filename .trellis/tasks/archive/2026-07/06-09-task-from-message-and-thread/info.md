...
chat message actions, task creation APIs, task source links
...
Task-from-message spec: prefilled source context, create from message/thread, link back from task to source.
...
```text
REAL_task_from_message_and_thread_<YYYYMMDDHHMMSS>
```
...
8. Write `evidence/REAL_task_from_message_and_thread_<timestamp>-notes.md` with commands run, pass/fail, and remaining gaps.
