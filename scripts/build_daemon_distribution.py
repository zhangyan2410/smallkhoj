...
preserve_previous() {
  local directory="$1"
  [[ -n "$active_path" && "$active_path" != "$directory" && -f "$ACTIVE_PATH" ]] || return 0
  local temporary="${STATE_ROOT}/previous.json.tmp-$$"
  cp "$ACTIVE_PATH" "$temporary"
  mv -f "$temporary" "${STATE_ROOT}/previous.json"
}
    ...
    install_script.write_text(
        "\n".join(
            [
                ...
                "$force = ($env:SMALLKHOJ_DAEMON_FORCE -eq '1' -or $env:AURA_DAEMON_FORCE -eq '1')",
                "$versionRoot = Join-Path (Join-Path $installRoot 'versions') ('v' + $version + '-' + $expectedPlatform)",
                "$binRoot = Join-Path $installRoot 'bin'",
                "$activePointerPath = Join-Path $installRoot 'active.json'",
                "$previousPointerPath = Join-Path $installRoot 'previous.json'",
                "function Get-AuraVersionKey([string]$value) {",
                "  $match = [regex]::Match($value, '^(?:v)?(\\d+)\\.(\\d+)\\.(\\d+)')",
                "  if (-not $match.Success) { throw \"Invalid Aura version: $value\" }",
                "  return New-Object Version ([int]$match.Groups[1].Value), ([int]$match.Groups[2].Value), ([int]$match.Groups[3].Value)",
                "}",
                "function Write-AuraJsonNoBom([string]$path, [object]$value) {",
                "  $temporary = $path + '.tmp-' + [Guid]::NewGuid().ToString('N')",
                "  $payload = $value | ConvertTo-Json",
                "  [IO.File]::WriteAllText($temporary, $payload, (New-Object Text.UTF8Encoding($false)))",
                "  Move-Item -LiteralPath $temporary -Destination $path -Force",
                "}",
                "function Install-AuraLaunchers {",
                "  $dispatcher = @'",
                "$ErrorActionPreference = 'Stop'",
                "$installRoot = Join-Path $env:LOCALAPPDATA 'Aura'",
                "$activePath = Join-Path $installRoot 'active.json'",
                "if (-not (Test-Path $activePath)) { throw 'Aura has no active release; rerun the installer.' }",
                "$active = Get-Content -LiteralPath $activePath -Raw | ConvertFrom-Json",
                "$target = Join-Path ([string]$active.path) 'aura.exe'",
                "if (-not (Test-Path $target)) { throw 'Aura active release is missing or incomplete; rerun the installer or use aura rollback.' }",
                "$env:AURA_STANDALONE = '1'",
                "$env:AURA_RELEASE_ROOT = [string]$active.path",
                "& $target @args",
                "exit $LASTEXITCODE",
                "'@",
                "  [IO.File]::WriteAllText((Join-Path $binRoot 'aura.ps1'), $dispatcher, (New-Object Text.UTF8Encoding($false)))",
                "  $launcherLines = @('@echo off', 'powershell.exe -NoProfile -ExecutionPolicy Bypass -File \"%~dp0aura.ps1\" %*')",
                ...
                "}",
                "function Test-AuraVersionComplete([string]$path) {",
                "  if (-not (Test-Path (Join-Path $path 'aura.exe'))) { return $false }",
                "  if (-not (Test-Path (Join-Path $path 'node.exe'))) { return $false }",
                "  if (-not (Test-Path (Join-Path $path 'dist\\cmd\\main.js'))) { return $false }",
                ...
                "  return ([string]$manifest.version -eq $version -and [string]$manifest.platform -eq $expectedPlatform)",
                "}",
                "New-Item -ItemType Directory -Force -Path (Split-Path $versionRoot), $binRoot | Out-Null",
                "# Clean up leftover staging/backup directories from previous failed installs so they",
                "# don't accumulate. Only touch our own '.staging-' / '.previous-' siblings.",
                "Get-ChildItem -LiteralPath (Split-Path $versionRoot) -Directory -Force -ErrorAction SilentlyContinue |",
                "  Where-Object { $_.Name -like '.staging-*' -or $_.Name -like '*.previous-*' } |",
                "  ForEach-Object { Remove-Item -LiteralPath $_.FullName -Recurse -Force -ErrorAction SilentlyContinue }",
                "$activePointer = $null",
                "if (Test-Path $activePointerPath) {",
                "  try { $activePointer = Get-Content -LiteralPath $activePointerPath -Raw | ConvertFrom-Json } catch { throw 'Aura active.json is invalid; run aura doctor before installing.' }",
                "}",
                "if ($activePointer -and $activePointer.version -and ((Get-AuraVersionKey ([string]$activePointer.version)) -gt (Get-AuraVersionKey $version)) -and -not $force) {",
                "  throw \"A newer Aura version ($($activePointer.version)) is already active; refusing to downgrade to $version. Use 'aura rollback --target-version $version' after the target version is installed, or set SMALLKHOJ_DAEMON_FORCE=1 for an explicit recovery install.\"",
                "}",
                "if ($activePointer -and [string]$activePointer.version -eq $version -and (Test-AuraVersionComplete $versionRoot)) {",
                "  Install-AuraLaunchers",
                "  Write-Output (\"Aura $version ($expectedPlatform) already-installed; archive download skipped.\")",
                ...
                "# staging directory whose final contents are moved onto the version root only",
                "# after the key files are confirmed present.",
                "# Stage BESIDE the final versionRoot (under versions/) with a short '.staging-<12hex>'",
                "# suffix, NOT under %TEMP%. The final Move-Item then becomes a same-directory rename",
                "# on the same volume, which is atomic and instantaneous even for a 20k-file tree.",
                "# Cross-directory Move-Item (TEMP -> versions) copies file-by-file and races with",
                "# real-time AV (Defender/CodexSandbox) scanning just-written files, producing",
                "# intermittent 'Access denied' (IOException) on the Move — observed on multiple hosts.",
                "$versionsRoot = Split-Path $versionRoot -Parent",
                "$staging = Join-Path $versionsRoot ('.staging-' + [Guid]::NewGuid().ToString('N').Substring(0,12))",
                "New-Item -ItemType Directory -Force -Path $staging | Out-Null",
                "$archivePath = Join-Path ([IO.Path]::GetTempPath()) ($artifactName)",
                "$backupRoot = $null",
                "try {",
                "  # Invoke-WebRequest's progress bar renders synchronously per chunk on PS 5.1 and",
                "  # dominates the download time for a large archive (~120x slower, e.g. 60s vs 0.5s",
                "  # for 136MB on localhost). Disable it for this scope so the download is bound by",
                "  # network/disk, not by progress rendering.",
                ...
                "  if ([string]$manifest.version -ne $version -or [string]$manifest.platform -ne $expectedPlatform) { throw 'The archive manifest version/platform does not match the requested release.' }",
                "  if (Test-Path $versionRoot) {",
                "    $backupRoot = $versionRoot + '.previous-' + [Guid]::NewGuid().ToString('N').Substring(0,8)",
                "    Move-Item -LiteralPath $versionRoot -Destination $backupRoot",
                "  }",
                "  # Move staging onto the version root with a short retry loop. Even though",
                "  # staging is now beside versionRoot (same-volume rename), a real-time AV",
                "  # scanner may briefly hold a handle on just-extracted files; retrying a",
                "  # few times lets it release instead of failing the whole install.",
                "  $moveError = $null",
                "  foreach ($attempt in 1..5) {",
                "    try {",
                "      Move-Item -LiteralPath $staging -Destination $versionRoot -ErrorAction Stop",
                "      $moveError = $null",
                "      break",
                "    } catch {",
                "      $moveError = $_",
                ...
                "  # Probe the new executable before switching active.json. A failed PE/runtime",
                "  # leaves the previous active pointer untouched and the old version directory intact.",
                "  & (Join-Path $versionRoot 'aura.exe') --version | Out-Null",
                "  if ($LASTEXITCODE -ne 0) { throw 'The staged Aura executable failed its version health probe.' }",
                "  if ($activePointer -and $activePointer.path -and ([string]$activePointer.path -ne [string]$versionRoot)) {",
                "    Write-AuraJsonNoBom $previousPointerPath $activePointer",
                "  }",
                "  $activeObject = @{ version = $version; platform = $expectedPlatform; path = $versionRoot }",
                "  Write-AuraJsonNoBom $activePointerPath $activeObject",
                "  Install-AuraLaunchers",
                "  if ($backupRoot -and (Test-Path $backupRoot)) { Remove-Item -LiteralPath $backupRoot -Recurse -Force -ErrorAction SilentlyContinue }",
                "  Write-Output (\"Installed Aura $version ($expectedPlatform) to $versionRoot\")",
                "  Write-Output (\"Aura is ready in this PowerShell: \" + (Join-Path $binRoot 'aura.cmd'))",
                "} catch {",
                "  if ($backupRoot -and (Test-Path $backupRoot)) {",
                "    if (Test-Path $versionRoot) { Remove-Item -LiteralPath $versionRoot -Recurse -Force -ErrorAction SilentlyContinue }",
                "    Move-Item -LiteralPath $backupRoot -Destination $versionRoot -Force -ErrorAction SilentlyContinue",
                "  }",
                "  throw",
                "} finally {",
