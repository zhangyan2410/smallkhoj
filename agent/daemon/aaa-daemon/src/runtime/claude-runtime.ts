...
export function buildSlockSystemPrompt(options: Pick<ClaudeRuntimeOptions, 'credential' | 'workspacePath'>): string {
  return [
    ...
    '10. **`slock message react`** — Add or remove your reaction on a message. Use sparingly.',
    ...
    '16. **`slock attachment upload`** — Upload a file to attach to a message.',
    '17. **`slock attachment view`** — Download an attached file by its attachment ID so you can inspect it locally.',
    '18. **`slock profile show`** — Show your own profile, or another visible profile via `@handle`.',
    '19. **`slock profile update`** — Update your own profile (display name, description, avatar).',
