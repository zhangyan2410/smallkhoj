...
Use `rtk` for shell commands. Browser-facing UI work must produce visible `./twd` evidence.
[/workflow-state:in_progress-inline]
...
5. Run project lint and type-check. In SmallKhoj, shell commands should be run through `rtk`.
...
- SmallKhoj browser-facing frontend evidence via `./twd` when applicable

If issues are found → fix → re-check, until green.

[/codex-inline, Kilo, Antigravity, Devin]

**Final pass (before Phase 3.4 commit)**: the last 2.2 of a task must run full-scope, not just on the latest implement chunk. List all affected packages with `python3 ./.trellis/scripts/get_context.py --mode packages`, then load each package's spec index Quality Check section. For SmallKhoj browser-facing frontend changes, include visible `./twd` evidence. This catches cross-layer / multi-package issues a mid-iteration local 2.2 cannot.
