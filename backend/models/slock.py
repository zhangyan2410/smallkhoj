...
class Message(Base):
    ...
    seq: Mapped[int] = mapped_column(
        BigInteger,
        # GENERATED BY DEFAULT AS IDENTITY: auto-assigns when INSERT omits `seq`
        # (unblocking plan 003-seq to drop the manual max(seq)+1 assignment), while
        # still permitting explicit overrides during the transition — current app
        # code (public_api / agent_api / reminder_scheduler) still passes seq=.
        # Use `always=False`, NOT `always=True`: ALWAYS would reject those explicit
        # inserts and break message posting until plan 003-seq lands.
