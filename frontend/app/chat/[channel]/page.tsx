...
export default function ChannelPage({ params }: { params: Promise<{ channel: string }> }) {
  ...
  useEffect(() => {
    let cancelled = false
    params.then(async ({ channel }) => {
      if (cancelled) return
      const decodedChannel = decodeChannelParam(channel)
  ...
  async function handleSend() {
    ...
    try {
      ...
      await fetch(`${API_BASE}/api/v1/channels/${encodedChannel}/messages`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "X-Public-Key": PUBLIC_KEY },
        body: JSON.stringify({ content: input.trim(), sender: "zy-ean" }),
      })
      setInput("")
      const msgsRes = await fetch(`${API_BASE}/api/v1/channels/${encodedChannel}/messages?limit=50`, {
  ...
  async function handleAddMember() {
    ...
    const activeChannelId =
      channelId || channels.find((channel) => channel.name.replace("#", "") === channelName)?.id || ""
    if (!memberId || !activeChannelId) return
    try {
      await apiPost(`/api/v1/channels/${activeChannelId}/members`, { memberId })
      if (addMemberSelectRef.current) addMemberSelectRef.current.value = ""
      const data = await apiGet<{ members: Member[] }>(`/api/v1/channels/${activeChannelId}/members`, { members: [] })
      setMembers(data.members || [])
    } catch (e) {
            ...
            <div className="border-t p-4">
              <div className="mx-auto flex max-w-3xl gap-2">
                <Input
                  ...
                  className="flex-1"
                />
                <Button aria-label="Send message" onClick={handleSend} disabled={!input.trim()}>
            ...
            >
              ...
              <div className="border-t pt-3 space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button size="sm" type="button" aria-label="Add member to channel" onClick={handleAddMember}>
