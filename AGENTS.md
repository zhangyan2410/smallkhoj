...
本仓库不使用 ZCode 的 plan 模式：**任何任务、任何情况下都不要调用 `EnterPlanMode`、不要请求或进入 plan 模式、不要生成平台计划文件**。需要先对齐方案时，直接在对话中给出简短方案要点并询问用户，确认后直接实施。规划职责由 Comet Native（Shape 阶段）或 Trellis 任务流程承担，普通任务直接内联规划。
