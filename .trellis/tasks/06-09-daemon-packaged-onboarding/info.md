...
connect command APIs, daemon packaging/start command, onboarding UI
...
Daemon onboarding spec: packaged command, one-time ticket safety, expiration, reconnect, troubleshooting, version/update UX.
...
```text
REAL_daemon_packaged_onboarding_<YYYYMMDDHHMMSS>
```
...
8. Write `evidence/REAL_daemon_packaged_onboarding_<timestamp>-notes.md` with commands run, pass/fail, and remaining gaps.
